...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-timestampindex", strprintf(_("Maintain a timestamp index for block hashes, used to query block hashes by range of timestamps (default: %u)"), DEFAULT_TIMESTAMPINDEX));
    ...
    string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, libevent, lock, mempool, mempoolrej, net, pow, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq"; // Don't translate these and qt below
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/emc2foundation/mil>";
