...
#if defined(NDEBUG)
# error "MIL cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "MIL Signed Message:\n";
...
char ASSETCHAINS_SYMBOL[65] = { "MIL" };
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s MIL additional fees, %d delta bytes\n",
...
bool WriteBlockToDisk(const CBlock& block, CDiskBlockPos& pos, const CMessageHeader::MessageStartChars& messageStart)
{
    ...
    LogPrintf("end WriteBlockToDisk:\n");
...
int static generateMTRandomWH2(uint64_t s, int range)
{
	boost::mt19937 gen(s);
	boost::uniform_int<> dist(1, range);
	return dist(gen);
...
CAmount GetBlockSubsidy(int nHeight, uint64_t nNonce, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = 0;
	...
	StartOffset = generateMTRandom(wseed, 27360);
	WormholeStartBlock = StartOffset + ((epoch - 1) * 36000); // Wormholes start from Epoch 2

	if (nHeight == 1)        nSubsidy = 1766000000 * COIN;
	else if (nHeight <= 130000)   nSubsidy = 16 * COIN;
	else if (nHeight <= 656000)  nSubsidy = 32 * COIN;
	else if (nHeight <= 1708000)  nSubsidy = 16 * COIN;
    else if (nHeight <= 3812000)  nSubsidy = 8 * COIN;
    else if (nHeight <= 8020000)  nSubsidy = 4 * COIN;
    else if (nHeight <= 16436000)  nSubsidy = 2 * COIN;
	else if (nHeight <= 66932000) nSubsidy = 1 * COIN;
	else nSubsidy = 0;

	if (epoch > 1 && epoch < 601 && nHeight >= WormholeStartBlock && nHeight < WormholeStartBlock + 8640) // 6 days
	{
		int chanche = 16;
		int RandomWH = generateMTRandomWH2(nNonce, 100)- chanche/2;

		if((nHeight % 100) - chanche / 2 < RandomWH && RandomWH < (nHeight % 100) + chanche / 2)
			nSubsidy += nSubsidy*(4320 - abs(nHeight - WormholeStartBlock - 4320))/10;
	}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight, pindex->pprev->nNonce, chainparams.GetConsensus());
    ...
    // For MIL also add the protocol rule that the first output in the coinbase must go to the charity address and have at least 20% of the subsidy (as per integer arithmetic)
    ...
    int64_t charityAmount = GetBlockSubsidy(pindex->nHeight, pindex->pprev->nNonce, chainparams.GetConsensus()) * 20 / 100;
    ...
    if(fTimestampIndex) {
        ...
        //retrieve logical timestamp of previous block
        if (pindex->pprev)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Milcoin: Check hardfork algo change!
    if(block.nHeight != nHeight){
        return state.Invalid(false, REJECT_INVALID, "bad-nHeight", "block nHeight is incorect");
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        //LogPrintf("net", "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString(), pfrom->id);
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            LogPrintf("more getheaders (%d) mixhash: %s to end to peer=%d (startheight:%d)\n", pindexLast->nHeight, pindexLast->mixhash.GetHex(), pfrom->id, pfrom->nStartingHeight);
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !pto->fDisconnect && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                LogPrintf("initial getheaders (%d) mixhash: %s to peer=%d (startheight:%d)\n", pindexStart->nHeight, pindexStart->mixhash.GetHex(), pto->id, pto->nStartingHeight);
